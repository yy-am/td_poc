        ...
        >
          ...
          <div class="node-body">
            ...
            <div v-if="node.tool_hints?.length" class="node-tools">
              <span v-for="tool in node.tool_hints" :key="tool" class="tool-tag">{{ tool }}</span>
            </div>
          </div>
          <div v-if="getDeps(node).length" class="node-deps">
            <span v-for="dep in getDeps(node)" :key="dep" class="dep-arrow">&#x2190; {{ dep }}</span>
          </div>
        </div>
      </div>
    </div>
...
function getDeps(node: PlanGraphNode): string[] {
  ...
  const nodeMap = new Map(props.planGraph.nodes.map(item => [item.id, item]))
