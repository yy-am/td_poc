  ...
  <div class="agent-detail-panel">
    ...
    <div v-if="event.metadata?.verdict" class="detail-section">
      ...
      <ul v-if="event.metadata.review_points?.length" class="review-list">
        <li v-for="(p, i) in event.metadata.review_points" :key="i">{{ p }}</li>
      </ul>
      <div v-if="event.metadata.issues?.length" class="issues">
        <div class="issue-title">发现的问题</div>
        <ul>
          <li v-for="(issue, i) in event.metadata.issues" :key="i" class="issue-item">{{ issue }}</li>
        </ul>
      </div>
      <div v-if="event.metadata.suggestions?.length" class="suggestions">
        <div class="suggest-title">改进建议</div>
        <ul>
          <li v-for="(s, i) in event.metadata.suggestions" :key="i">{{ s }}</li>
        </ul>
      </div>
    </div>
    ...
    <div v-if="event.metadata?.evidence?.length" class="detail-section">
      ...
      <ul class="evidence-list">
        <li v-for="(e, i) in event.metadata.evidence" :key="i">{{ e }}</li>
      </ul>
    </div>
  </div>
</template>
...
const agentLabel = computed(() => {
  const map: Record<string, string> = {
    planner: 'Planner', executor: 'Executor', reviewer: 'Reviewer', orchestrator: 'System',
  }
...
const typeLabel = computed(() => {
  const map: Record<string, string> = {
    agent_start: '开始', plan: '计划', plan_update: '更新计划',
    thinking: '思考', action: '执行', observation: '观察结果',
    review: '审查', replan_trigger: '重规划', answer: '最终报告',
    error: '错误', status: '状态',
  }
...
const tableRows = computed(() => {
  const td = props.event.metadata?.table_data
  if (!td?.columns || !td?.rows) return []
  return td.rows.map((row: any) => {
    if (Array.isArray(row)) {
      const obj: Record<string, any> = {}
      td.columns.forEach((col: string, i: number) => { obj[col] = row[i] })
      return obj
    }
    return row
  })
...
.detail-agent-tag.agent-planner { background: rgba(56,139,253,0.2); color: #58a6ff; }
.detail-agent-tag.agent-executor { background: rgba(255,166,87,0.2); color: #ffa657; }
.detail-agent-tag.agent-reviewer { background: rgba(63,185,80,0.2); color: #3fb950; }
.detail-agent-tag.agent-orchestrator { background: rgba(139,148,158,0.2); color: #8b949e; }
.detail-type { font-size: 12px; color: #8b949e; }
.detail-duration { font-size: 11px; color: #6e7681; margin-left: auto; }
.detail-content {
  font-size: 13px;
  color: #c9d1d9;
  line-height: 1.6;
...
.section-body { font-size: 12px; color: #c9d1d9; line-height: 1.6; }
.reasoning { font-style: italic; color: #a5b4c7; }
.tool-info { display: flex; gap: 8px; align-items: center; margin-bottom: 6px; }
.tool-name {
  font-weight: 600;
  color: #ffa657;
  font-size: 12px;
}
.tool-summary { font-size: 11px; color: #8b949e; }
.sql-block {
  background: #161b22;
  border-radius: 6px;
  padding: 8px 12px;
...
.verdict-badge.approve { background: rgba(63,185,80,0.2); color: #3fb950; }
.verdict-badge.reject { background: rgba(248,81,73,0.2); color: #f85149; }
.review-list, .evidence-list {
  padding-left: 16px;
  font-size: 12px;
  color: #c9d1d9;
}
.review-list li, .evidence-list li { margin: 2px 0; }
.issues { margin-top: 6px; }
.issue-title, .suggest-title {
  font-size: 11px;
  font-weight: 600;
  color: #f85149;
  margin-bottom: 4px;
}
.suggest-title { color: #d29922; }
.issue-item { color: #f85149; }
.suggestions { margin-top: 6px; }
.suggestions li { font-size: 12px; color: #d29922; }
.result-table {
  margin-top: 6px;
}
:deep(.el-table) {
