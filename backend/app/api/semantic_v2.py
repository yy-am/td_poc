...
from app.schemas.semantic import SemanticModelSummary, SemanticQueryRequest
from app.semantic.compiler import SemanticDefinitionError
from app.semantic.service_v2 import semantic_query as execute_semantic_query
...
async def list_models(db: AsyncSession = Depends(get_db)):
    ...
    return [SemanticModelSummary.model_validate(m) for m in models]
...
async def get_catalog(db: AsyncSession = Depends(get_db)):
    ...
    for m in models:
        entry = SemanticModelResponse.model_validate(m).model_dump()
        catalog.setdefault(m.model_type, catalog["physical"]).append(entry)

    table_stats: dict[str, int] = {}
    for m in models:
        try:
            count_result = await db.execute(text(f'SELECT COUNT(*) FROM "{m.source_table}"'))
            table_stats[m.source_table] = int(count_result.scalar() or 0)
        except Exception:
            table_stats[m.source_table] = 0
